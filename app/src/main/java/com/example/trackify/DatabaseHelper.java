...
//                        "amount REAL, " +
//                        "category TEXT, " +
//                        "date TEXT, " +  // store date in yyyy-MM-dd format
//                        "type TEXT, " +
//                        "description TEXT, " +
//                        "FOREIGN KEY(user_id) REFERENCES " + TABLE_USERS + "(id))"
//        );
...
//        values.put("username", username);
//        values.put("password", password);
//        values.put("date_of_joining", currentDate); // added
//        values.put("last_login", currentDate);      // added
//        long result = db.insert(TABLE_USERS, null, values);
//        return result != -1;
//    }
//
...
//        if (cursor.moveToFirst()) {
//            userId = cursor.getInt(0);
//            // Update last_login to current date
//            ContentValues values = new ContentValues();
//            values.put("last_login", new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(new Date()));
//            db.update(TABLE_USERS, values, "id = ?", new String[]{String.valueOf(userId)});
//        }
...
//        values.put("amount", amount);
//        values.put("category", category);
//        values.put("date", date);  // Expecting yyyy-MM-dd
//        values.put("type", type);
//        values.put("description", description);
//        long result = db.insert(TABLE_TRANSACTIONS, null, values);
//        return result != -1;
...
//    public double getTotalByType(int userId, String type) {
//        SQLiteDatabase db = this.getReadableDatabase();
//        Cursor cursor = db.rawQuery("SELECT SUM(amount) FROM " + TABLE_TRANSACTIONS + " WHERE user_id=? AND type=?", new String[]{String.valueOf(userId), type});
//        double total = 0;
//        if (cursor.moveToFirst()) {
//            total = cursor.getDouble(0);
//        }
...
//        SQLiteDatabase db = this.getReadableDatabase();
//        Cursor cursor = db.rawQuery("SELECT * FROM " + TABLE_TRANSACTIONS + " WHERE id=?", new String[]{String.valueOf(transactionId)});
//        Transaction transaction = null;
//        if (cursor.moveToFirst()) {
//            transaction = new Transaction();
//            transaction.setId(cursor.getInt(cursor.getColumnIndexOrThrow("id")));
...
//        List<Transaction> transactions = new ArrayList<>();
//        SQLiteDatabase db = this.getReadableDatabase();
//        Cursor cursor = db.rawQuery("SELECT * FROM " + TABLE_TRANSACTIONS + " WHERE user_id=?", new String[]{String.valueOf(userId)});
//        if (cursor.moveToFirst()) {
//            do {
//                Transaction transaction = new Transaction();
//                transaction.setId(cursor.getInt(cursor.getColumnIndexOrThrow("id")));
...
//        switch (period.toLowerCase()) {
//            case "day":
//                query = "SELECT SUM(amount) FROM transactions WHERE user_id = ? AND type = 'expense' AND date = ?";
//                args = new String[]{String.valueOf(userId), today};
//                break;
//            case "week":
//                query = "SELECT SUM(amount) FROM transactions WHERE user_id = ? AND type = 'expense' AND date >= DATE(?, '-6 days')";
...
//        SQLiteDatabase db = this.getReadableDatabase();
//
//        Cursor cursor = db.rawQuery("SELECT * FROM transactions WHERE user_id=? ORDER BY date DESC LIMIT ?",
...
//        SQLiteDatabase db = this.getReadableDatabase();
//
//        String dateCondition = "date = date('now', 'localtime')";
//        switch (period) {
//            case "daily":
//                dateCondition = "date = date('now', 'localtime')";
//                break;
...
//        return list;
//    }
//    // Get total amount grouped by category for user and type (income or expense)
//
//}
